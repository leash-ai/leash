...
function Tick({ done, label }: { done: boolean; label: string }) {
  return (
    <div className="flex items-center gap-2">
      <span className={done ? "text-best" : "text-ink-faint"}>{done ? "●" : "○"}</span>
      <span className={`text-xs font-mono ${done ? "text-ink-dim" : "text-ink-faint"}`}>
...
export function SettlementPanel({ duelId, stake, settlement, now, onResolved }: Props) {
  ...
  return (
    <div className="border border-track-line rounded-lg p-6 bg-track-soft">
      {windowOpen ? (
        <>
          <div className="flex items-baseline justify-between mb-4">
            <div className="text-best text-xs font-mono">SETTLEMENT WINDOW</div>
            <div className="font-mono text-sm text-ink-dim">{secondsLeft}s left</div>
          </div>
        ...
        <>
          <div className="text-best text-xs font-mono mb-2">READY TO RESOLVE</div>
          <p className="text-xs text-ink-faint font-mono mb-4">
            ...
            <span className="text-ink-dim">{ethers.formatEther(bonus)} COTI</span> for the gas.
          </p>
          ...
          <button
            onClick={resolve}
            disabled={resolving}
            className="bg-best text-black font-mono text-sm px-4 py-2 rounded hover:bg-[#00e57a] disabled:opacity-50 disabled:cursor-not-allowed"
          >
